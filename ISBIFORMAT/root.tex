...
%
\makeatletter
\def\@name{\emph{Hastings Greer$^{\star}$ \quad Sam Gerber$^{\star}$ \quad Marc Niethammer$^{\dagger}$ \quad Roland Kwitt$^{\circ}$ } \\ \emph{ Matt McCormick$^{\star}$ \quad  Deepak Chittajallu$^{\star}$ \quad Neal Siekierski$^{\star}$ } \\ \emph{ Matthew Oetgen$^{\otimes}$ \quad Kevin Cleary$^{\otimes}$ \quad Stephen Aylward$^{\star}$ } }
\makeatother
